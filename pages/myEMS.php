...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // attr_time은 저녁 미션 설정이므로 '저녁'으로 고정
    $attrTime = '저녁';
    ...
    if ($stmt->execute()) {
        // 데이터 저장 성공 시 emsOK.php로 리다이렉트
        header("Location: emsOK.php");
        exit;
    } else {
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>마이페이지-미션 설정-저녁 미션 설정</title>
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no">
        <link rel="stylesheet" href="myEMS.css">
        <!-- ✅ Noto Sans KR 웹폰트 추가 -->
    ...
    <body>
        ...
        <form action="myEMS.php" method="POST">
            <div id="nameInput">
                <h3>이름</h3>
